// pages-watchout.jsx — the /watch-out help page.
...
function WatchOutPage({ navigate }) {
  const [active, setActive] = React.useState(WATCHOUT_SECTIONS[0].id);
        ...
        <article className="watchout-body">
          ...
          <section id="beta-status" className="watchout-sec">
            ...
            <Callout tone="yellow" icon="!" title="rustygit v0.1.0 is beta">
              <p>
                ...
                repos. <strong>Don't make it load-bearing for irreplaceable state
                without a backup.</strong>
              </p>
          ...
          <section id="unsupported" className="watchout-sec">
            ...
            <p className="watchout-lede">
              ...
              refuse clearly; <strong>use upstream <code>git</code> for that
              operation.</strong>
            </p>
            <div className="bigsix">
              {[
                { name: "Submodules", body: <p><code>submodule add</code> / <code>update</code> / <code>foreach</code> are not implemented. Repos <em>containing</em> submodules clone and check out fine (gitlink mode 160000 is preserved) — but you can't manage the submodules with rustygit.</p> },
                { name: "Sparse-checkout", body: <p>Cone and non-cone both unimplemented.</p> },
                { name: ".gitattributes filters", body: <p>Smudge / clean / textconv filters are not run. <strong>Therefore Git LFS — LFS repos must stay on upstream <code>git</code>.</strong></p> },
                { name: "Interactive rebase", body: <p><code>rebase -i</code>, <code>--autosquash</code>, <code>--rebase-merges</code>, <code>--exec</code> are not implemented. Non-interactive <code>rebase &lt;upstream&gt;</code> works.</p> },
                { name: "Partial clone", body: <p><code>--filter=blob:none</code>, promisor remotes. rustygit silently does a full clone instead of erroring — you lose the bandwidth savings.</p> },
                { name: "Old transports", body: <p><code>git://</code>, <code>ftp://</code>, <code>ftps://</code>, <code>rsync://</code>, and protocol v0/v1 are refused with a named error. v2 over HTTPS is the only supported transport.</p> },
              ].map((it, i) => (
              ...
              <article className="danger">
                ...
                <div className="danger-content">
                  ...
                  <p>
                    ...
                    <strong>Don't open repos from untrusted sources without
                    ulimits.</strong>
                  </p>
                </div>
              </article>
...
rgs() { rustygit status "$@" && rustygit diff --stat "$@"; }`}</CodeBlock>
          ...
          <section id="filing-bugs" className="watchout-sec">
            ...
            <p>
              ...
              <a href="https://github.com/bsadashi/rustygit/issues" target="_blank" rel="noreferrer">
                github.com/bsadashi/rustygit/issues
              </a>.
            </p>
            <Callout tone="red" icon="!!" title="Data-loss / silent-corruption / segfault">
              <p>
                Use <a href="https://github.com/bsadashi/rustygit/security/advisories/new" target="_blank" rel="noreferrer">GitHub Security Advisories</a> instead of a public issue, so the
                discussion stays private until a fix ships.
              </p>
            </Callout>
          </section>
        </article>
...
Object.assign(window, { WatchOutPage });
