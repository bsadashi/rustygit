...
1. **Install** — three tabs:
   - **From source** (works today):
     ...
     cargo install --path .
     ```
   - **Homebrew** (coming in v0.1.0 final; designer: render as
     "Coming soon — see release notes" until the tap is live):
     ```sh
     brew install bsadashi/tap/rustygit
     ```
   - **`.deb` / `.rpm`** (coming in v0.1.0 final): link to GitHub
     Releases.
